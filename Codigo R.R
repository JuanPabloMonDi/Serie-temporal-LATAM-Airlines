...
data_fim  <- Sys.Date()-1 # Data de fim
...
for (i in 1:7){
  ...
  mod<-modelos[[i]]
  errores<-t(data.frame(cross_val(retornos_c_antes_pandemia,mod,window_size = 400))) 
  rownames(errores)<-i
...
cv_pospandemia<-rbind(cv_pospandemia,t(data.frame(cross_val(precos1$retornos_c,mod8,window_size = 200,xreg=precos1$depois_da_pandemia))))
cv_pospandemia<-rbind(cv_pospandemia,t(data.frame(cross_val(precos1$retornos_c,mod9,window_size = 200, xreg=precos1$depois_da_pandemia))))
cv_pospandemia<-rbind(cv_pospandemia,t(data.frame(cross_val(precos1$retornos_c,mod10,window_size = 200,xreg=precos1$depois_da_pandemia))))
cv_pospandemia<-rbind(cv_pospandemia,t(data.frame(cross_val(precos1$retornos_c,mod11,window_size = 200, xreg=precos1$depois_da_pandemia))))
cv_pospandemia<-rbind(cv_pospandemia,t(data.frame(cross_val(precos1$retornos_c,mod12,window_size = 200, xreg=precos1$depois_da_pandemia))))
cv_pospandemia<-rbind(cv_pospandemia,t(data.frame(cross_val(precos1$retornos_c,mod13,window_size = 200,xreg=precos1$depois_da_pandemia,differentation = 1))))
